...
export default function EditTicketComment(props) {
  // router object from next
  const router = useRouter();

  const ticketCommentId = router.query.ticketCommentId;

  const updateTicketComment = async (event) => {
    ...
    router.push(`/ticketComments/${ticketCommentId}`)

  };

  const deleteTicketComment = async (event) => {
    event.preventDefault();
    ...
    <div className="bg-gray-700 h-screen">
      <div className="p-2">
        <div className=" m-4 p-4 rounded-md bg-gray-400 font-light space-y-8 ">
          <form onSubmit={updateTicketComment}>
            <div>
              <h3 className="text-4xl leading-6 font-light text-gray-900">Edit TicketComment</h3>
            </div>
        ...
        <div className="mt-1 flex rounded-md shadow-sm">
          <input
          defaultValue={props.ticketComment.ticket}
            type="text"
            name="ticket"
            id="ticket"
            autoComplete="ticket"
        ...
        <div className="mt-1 flex rounded-md shadow-sm">
          <input
          defaultValue={props.ticketComment.content}
            type="text"
            name="content"
            id="content"
        ...
        <div className="mt-1 flex rounded-md shadow-sm">
          <input
          defaultValue={props.ticketComment.author}
            type="text"
            name="author"
            id="author"
            autoComplete="author"
